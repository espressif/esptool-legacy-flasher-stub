...
int handle_flash_erase(uint32_t addr, uint32_t len)
{
    ...
    while (len > 0 && (addr % FLASH_BLOCK_SIZE != 0)) {
        ...
        if (large_flash_mode) {
            if (esp_rom_opiflash_erase_sector(addr / FLASH_SECTOR_SIZE) != 0) {
                return 0x35;
            }
        } else {
            if (SPIEraseSector(addr / FLASH_SECTOR_SIZE) != 0) {
                return 0x35;
            }
        }
#elif ESP32P4
...
#if defined(ESP32S3) && !defined(ESP32S3BETA2)
        if (large_flash_mode) {
            if (esp_rom_opiflash_erase_block_64k(addr / FLASH_BLOCK_SIZE) != 0) {
                return 0x36;
            }
        } else {
            if (SPIEraseBlock(addr / FLASH_BLOCK_SIZE) != 0) {
                return 0x36;
            }
        }
#elif ESP32P4
